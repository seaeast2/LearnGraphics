...
void Rasterization::Render(vector<vec4> &pixels) {
    ...
    const auto yMax = size_t(glm::clamp(glm::ceil(std::max({v0.y, v1.y, v2.y})), 0.0f,  float(width - 1)));
    ...
    for (size_t j = yMin; j <= yMax; j++) {
        for (size_t i = xMin; i <= xMax; i++) {
            ...
            if (alpha0 >= 0 && alpha1 >= 0 && alpha2 >= 0) {
                ...
                const vec3 color = vec3(w0 * triangle.v0.color + w1 * triangle.v1.color + w2 * triangle.v2.color);
